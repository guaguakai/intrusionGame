...
public class NewMadagascar {
	...
	public static String PQSearch(AbstractBaseGraph graph, double budget, int t, boolean pad) throws MalformedGraphException, LPSolverException{
		...
		String results = budget +", "+result.getEvaluation() + ", "+(tock_pq-tick_pq)/1000 + ", " 
				+result.resourcesList.get(0)+", "+result.resourcesList.get(1)+", "+result.resourcesList.get(2)+", "+result.resourcesList.get(3);
		return results;
	}
	...
	public static String HeuristicGuarantee(AbstractBaseGraph graph, int budget, 
		...
		String results = budget +", "+root.getEvaluation() + ", "+(tock_guar-tick_guar)/1000 + ", "+(tock_tot_guar-tick_guar)/1000 + ", "
				+root.resourcesList.get(0)+", "+root.resourcesList.get(1)+", "+root.resourcesList.get(2)+", "+root.resourcesList.get(3)+", "
				+Integer.toString(tot_cost[0])+", "+Integer.toString(tot_cost[1])+", "+Integer.toString(tot_cost[2])+", "+Integer.toString(tot_cost[3])+", "
				+Integer.toString(tot_cov[0])+", "+Integer.toString(tot_cov[1])+", "+Integer.toString(tot_cov[2])+", "+Integer.toString(tot_cov[3])+", "
				+Double.toString(tot_prob[0])+", "+Double.toString(tot_prob[1])+", "+Double.toString(tot_prob[2])+", "+Double.toString(tot_prob[3]);
	...
	public static String EvalTeamGuarantee(AbstractBaseGraph graph, double budget, int t, boolean pad,
		...
		String results = budget+", "+ root_sol.getEvaluation() + ", "
				...
				+bestTeam[0]+", "+bestTeam[1]+", "+bestTeam[2]+", "+bestTeam[3];
		return results;
	}
	...
	public static String EvalTeamFast(AbstractBaseGraph graph, double budget, int t, boolean pad,
		...
		for(int ii=0;ii<n_teams;ii++){
			...
			for(int jj=0;jj<len;jj++){
				if(curr_team[jj] == 1)
					team_values[ii] += resource_values[jj];
			}
		}
	...
	public static void main(String[] args) throws MalformedGraphException, LPSolverException ,FileNotFoundException{
		...
		int budget = 10;
		int [] resourcecoverage = {2,3,2,3};
		double [] resourceprob = {0.7,0.6,0.9,0.8};
		int [] resourcecost = {5,5,8,8};
		...
		for(;budget<41;budget+=10){
			...
			try {
				PrintWriter out_fast = new PrintWriter(new BufferedWriter(new FileWriter(
						"results_with_prob_madagascar_fast_budget_"+budget+".csv", true)));
			...
			try {
				PrintWriter out_evfast_div = new PrintWriter(new BufferedWriter(new FileWriter(
						"results_with_prob_madagascar_evalfast_div_budget_"+budget+".csv", true)));
			...
			try {
				PrintWriter out_evfast_no_div = new PrintWriter(new BufferedWriter(new FileWriter(
						"results_with_prob_madagascar_evalfast_no_div_budget_"+budget+".csv", true)));
			...
			try {
				PrintWriter out_guarantee_div = new PrintWriter(new BufferedWriter(new FileWriter(
						"results_with_prob_madagascar_guarantee_div_budget_"+budget+".csv", true)));
				out_guarantee_div.println(results_guarantee_div);
				out_guarantee_div.close();
			} catch (IOException e) {
			}

			String results_guarantee_no_div = HeuristicGuarantee(graph, budget, resourcecoverage, resourceprob, resourcecost,false);
			try {
				PrintWriter out_guarantee_no_div = new PrintWriter(new BufferedWriter(new FileWriter(
						"results_with_prob_madagascar_guarantee_no_div_budget_"+budget+".csv", true)));
				out_guarantee_no_div.println(results_guarantee_no_div);
				out_guarantee_no_div.close();
			} catch (IOException e) {
			}
			...
			try {
				PrintWriter out_evguar_div = new PrintWriter(new BufferedWriter(new FileWriter(
						"results_with_prob_madagascar_evalguar_div_budget_"+budget+".csv", true)));
			...
			teamNode.setTeams(resourcecost,resourcecoverage,resourceprob,true);
			String results_evguar_no_div = EvalTeamGuarantee(graph, budget, 1, false, resourcecoverage, resourceprob, resourcecost,false);	
			try {
				PrintWriter out_evguar_no_div = new PrintWriter(new BufferedWriter(new FileWriter(
						"results_with_prob_madagascar_evalguar_no_div_budget_"+budget+".csv", true)));
				out_evguar_no_div.println(results_evguar_no_div);
				out_evguar_no_div.close();
			} catch (IOException e) {
			}
			...
			try {
				PrintWriter out_opt = new PrintWriter(new BufferedWriter(new FileWriter(
						"results_with_prob_madagascar_opt_budget_"+budget+".csv", true)));
