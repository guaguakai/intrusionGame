...
public class Random50 {
	...
	public static String HeuristicGuarantee(AbstractBaseGraph graph, int budget, 
		...
		for(int ii=0;ii<len;ii++){
			...
			try {
				...
				computed_values[ii] = root.getEvaluation();
			} catch (MalformedGraphException e) {
				e.printStackTrace();
			} catch (LPSolverException e) {
				e.printStackTrace();
		...
		while(ii < len){
			if(decremented_budget-tot_cost[ii] >= 0){
				num_resources[ii] += 1;
				decremented_budget -= tot_cost[ii];
			} else {
				ii++;
			}
		}

		tock_guar = System.currentTimeMillis();
		...
		String results = "";
		if(root.getEvaluation() >= max_comp_value)
			results = budget +", "+root.getEvaluation()+ ", "+(tock_guar-tick_guar)/1000 + ", "+(tock_tot_guar-tick_guar)/1000 + ", "
			...
			results = budget +", "+max_comp_value+ ", "+(tock_guar-tick_guar)/1000 + ", "+(tock_tot_guar-tick_guar)/1000 + ", "
					+root.resourcesList.get(max_index)+", "+Integer.toString(tot_cost[max_index])+", "
					+Integer.toString(tot_cov[max_index])+", "+Double.toString(tot_prob[max_index]);
		return results;
	}


	//	public static String HeuristicGuarantee(AbstractBaseGraph graph, int budget, 
	//			int[] resourcecoverage, double[] resourceprob, int[] resourcecost, boolean div, int maxres){
	//		double tick_guar = System.currentTimeMillis();
	//		double tock_guar = 0;
	//		double tock_tot_guar = 0;
	//		int len = resourcecost.length;
	...
	//				max = -99999;
	//			}
	//			//Here we have the resources ordered according to the HeuristicFast
	//			//We copy only the maxres resources we can manage
	//
	//			tot_cost = Arrays.copyOfRange(tot_cost, 0, maxres);
	//			tot_prob = Arrays.copyOfRange(tot_prob, 0, maxres);
	//			tot_cov = Arrays.copyOfRange(tot_cov, 0, maxres);
	//			len = tot_cost.length;
	//
	//			//Now we evaluate the maxres resources according to their real utility
	//
	//			for(int ii=0;ii<len;ii++){
	//				int[] curr_cov = {tot_cov[ii]};
	//				double[] curr_prob = {tot_prob[ii]};
	//				int[] curr_cost = {tot_cost[ii]};
	//				teamNode.setTeams(curr_cost,curr_cov,curr_prob,useProbabilities);
	//				teamNode root = new teamNode("root", null, 0, budget, false);
	//				root.teamObj.setGraph(graph);			
	//				root.setResources(resources);
	...
	//					root.setFullBetterRuggedEvaluation();
	//					if(div)
	//						values[ii] = root.getEvaluation()/tot_cost[ii];
	//					else
	//						values[ii] = root.getEvaluation();
	//				} catch (MalformedGraphException e) {
	//					e.printStackTrace();
	//				} catch (LPSolverException e) {
	//					e.printStackTrace();
	...
	//			max_index = 0;
	//
	//			int[] cost_temp = tot_cost;
	//			double[] prob_temp = tot_prob;
	//			int[] cov_temp = tot_cov;
	//
	//			for(int ii=0;ii<len;ii++){
	//				for (int jj = 0; jj < len; jj++) {
	//					if (values[jj] > max) {
	...
	//					}
	//				}
	//				tot_cov[ii] = cov_temp[max_index];
	//				tot_prob[ii] = prob_temp[max_index];
	//				tot_cost[ii] = cost_temp[max_index];
	//
	//				values[max_index] = -99999.99;
	//				max = -99999;
	//			}	
	...
	//					root.setFullBetterRuggedEvaluation();
	//					if(div)
	//						values[ii] = root.getEvaluation()/resourcecost[ii];
	//					else
	//						values[ii] = root.getEvaluation();
	//				} catch (MalformedGraphException e) {
	//					e.printStackTrace();
	...
	//
	//		String results;
	//		if(maxres < resourcecoverage.length){
	//			results = budget +", "+root.getEvaluation() + ", "+(tock_guar-tick_guar)/1000 + ", "+(tock_tot_guar-tick_guar)/1000 + ", "
	//					+root.resourcesList.get(0)+", "+root.resourcesList.get(1)+", "+root.resourcesList.get(2)
	//					+Integer.toString(tot_cost[0])+", "+Integer.toString(tot_cost[1])+", "+Integer.toString(tot_cost[2])+", "
	//					+Integer.toString(tot_cov[0])+", "+Integer.toString(tot_cov[1])+", "+Integer.toString(tot_cov[2])+", "
	...
	public static String HeuristicFast(AbstractBaseGraph graph, int budget, 
		...
		tock_fast =  System.currentTimeMillis();
		...
		String results = budget +", "+root.getEvaluation() + ", "+(tock_fast-tick_fast)/1000 + ", "+(tock_tot_fast-tick_fast)/1000 + ", "
				+root.resourcesList.get(0)+", "+root.resourcesList.get(1)+", "+root.resourcesList.get(2)+", "
				+root.resourcesList.get(3)+", "+root.resourcesList.get(4)+", "+root.resourcesList.get(5)+", "
				+Integer.toString(tot_cost[0])+", "+Integer.toString(tot_cost[1])+", "+Integer.toString(tot_cost[2])+", "
				+Integer.toString(tot_cost[3])+", "+Integer.toString(tot_cost[4])+", "+Integer.toString(tot_cost[5])+", "
	...
	public static String EvalTeamGuarantee(AbstractBaseGraph graph, double budget, int t, boolean pad,
			int[] resourcecoverage, double[] resourceprob, int[] resourcecost, boolean div) throws MalformedGraphException, LPSolverException{
		double tick_evguar = System.currentTimeMillis();
		double tock_evguar = 0;
		...
		for(int ii=0;ii<len;ii++){
			...
			try {
				root.setFullBetterRuggedEvaluation();
				if(div)
					resource_values[ii] = root.getEvaluation()/resourcecost[ii];
				else
					resource_values[ii] = root.getEvaluation();
			} catch (MalformedGraphException e) {
				e.printStackTrace();
			} catch (LPSolverException e) {
				e.printStackTrace();
		...
		for(int ii=0;ii<n_teams;ii++){
			...
			for(int jj=0;jj<len;jj++){
				//				if(curr_team[jj] > 0)
				team_values[ii] += resource_values[jj]*curr_team[jj];
			}
		}
		...
		tock_evguar =  System.currentTimeMillis();
		...
		String results = budget+", "+ root_sol.getEvaluation() + ", "
				+(tock_evguar-tick_evguar)/1000 + ", "+(tock_tot_evguar-tick_evguar)/1000 + ", "
	...
	//ERRORS IN ORDERING ACCORDING TO THE FAST, PROBLEMS OF INDICES
	//	public static String EvalTeamGuarantee(AbstractBaseGraph graph, double budget, int t, boolean pad,
	//			int[] resourcecoverage, double[] resourceprob, int[] resourcecost, boolean div, int maxres) throws MalformedGraphException, LPSolverException{
	//		double tick_evguar = System.currentTimeMillis();
	//		double tock_evguar = 0;
	//		double tock_tot_evguar = 0;
	//		int len = resourcecost.length;
	...
	//			}
	//
	//			double max = resource_values[0];
	//			int[] max_indices = new int[len];
	//			int counter = 0;
	//
	//			for(int ii=0;ii<maxres;ii++){
	//				for (int jj = 0; jj < len; jj++) {
	//					if (resource_values[jj] > max) {
	//						max_indices[counter] = jj;
	//					}else if(resource_values[jj] == max){
	//						if(counter > 0){
	//							if(resourcecoverage[jj] > resourcecoverage[max_indices[counter-1]]){
	//								max_indices[counter] = jj;
	//							}
	//						}
	//					}			
	//				}
	//				counter++;
	//				resource_values[max_indices[counter-1]] = -99999.99;
	//				max = -99999;
	//			}
	//
	//			//Now we evaluate the maxres resources according to their real utility
	//			Arrays.fill(resource_values, -99999.99);
	//			for(int ii=0;ii<maxres;ii++){
	//				int[] curr_cov = {resourcecoverage[max_indices[ii]]};
	//				double[] curr_prob = {resourceprob[max_indices[ii]]};
	//				int[] curr_cost = {resourcecost[max_indices[ii]]};
	//				teamNode.setTeams(curr_cost,curr_cov,curr_prob,useProbabilities);
	//				teamNode root = new teamNode("root", null, 0, budget, false);
	//				root.teamObj.setGraph(graph);			
	//				root.setResources(resources);
	...
	//					root.setFullBetterRuggedEvaluation();
	//					if(div)
	//						resource_values[max_indices[ii]] = root.getEvaluation()/resourcecost[max_indices[ii]];
	//					else
	//						resource_values[max_indices[ii]] = root.getEvaluation();
	//				} catch (MalformedGraphException e) {
	//					e.printStackTrace();
	//				} catch (LPSolverException e) {
	//					e.printStackTrace();
	...
	//
	//			for(int ii=0;ii<len;ii++){
	//				if(resource_values[ii] < -90000)
	//					resource_values[ii] = 0;
	//			}
	//
	//		}else{	
	//			for(int ii=0;ii<len;ii++){
	...
	public static String EvalTeamFast(AbstractBaseGraph graph, double budget, int t, boolean pad,
		...
		int[] bestTeam = teamList.get(max_index).resources;
		tock_evfast =  System.currentTimeMillis();
		...
		String results = budget+", "+ root_sol.getEvaluation() + ", "
				+(tock_evfast-tick_evfast)/1000 + ", "+(tock_tot_evfast-tick_evfast)/1000 + ", " 
				+bestTeam[0]+", "+bestTeam[1]+", "+bestTeam[2]+", "
				+bestTeam[3]+", "+bestTeam[4]+", "+bestTeam[5];
		return results;
	}
	...
	public static void main(String[] args) throws MalformedGraphException, LPSolverException ,FileNotFoundException{
		...
		teamNode.setTeams(resourcecost,resourcecoverage,resourceprob,true);
			...
			AbstractBaseGraph graph = RandomGraph(40, 4, 4, 0.1);
			teamNode.setTeams(resourcecost,resourcecoverage,resourceprob,true);
			String results_fast = HeuristicFast(graph, budget, resourcecoverage, resourceprob, resourcecost);
			try {
				PrintWriter out_fast = new PrintWriter(new BufferedWriter(new FileWriter(
						"results_with_prob_random_40_fast_budget_"+budget+".csv", true)));
				out_fast.println(results_fast);
				out_fast.close();
			} catch (IOException e) {
			}

			teamNode.setTeams(resourcecost,resourcecoverage,resourceprob,true);
			String results_evfast_div = EvalTeamFast(graph, budget, 1, false, resourcecoverage, resourceprob, resourcecost,true);	
			try {
				PrintWriter out_evfast_div = new PrintWriter(new BufferedWriter(new FileWriter(
						"results_with_prob_random_40_evalfast_div_budget_"+budget+".csv", true)));
				out_evfast_div.println(results_evfast_div);
				out_evfast_div.close();
			} catch (IOException e) {
			}

			String results_evfast_no_div = EvalTeamFast(graph, budget, 1, false, resourcecoverage, resourceprob, resourcecost,false);	
			try {
				PrintWriter out_evfast_no_div = new PrintWriter(new BufferedWriter(new FileWriter(
						"results_with_prob_random_40_evalfast_no_div_budget_"+budget+".csv", true)));
				out_evfast_no_div.println(results_evfast_no_div);
				out_evfast_no_div.close();
			} catch (IOException e) {
			}

			String results_guarantee_div = HeuristicGuarantee(graph, budget, resourcecoverage, resourceprob, resourcecost,true);
			try {
				PrintWriter out_guarantee_div = new PrintWriter(new BufferedWriter(new FileWriter(
						"results_with_prob_random_40_guarantee_div_budget_"+budget+".csv", true)));
				out_guarantee_div.println(results_guarantee_div);
				out_guarantee_div.close();
			} catch (IOException e) {
			}

			teamNode.setTeams(resourcecost,resourcecoverage,resourceprob,true);
			String results_evguar_div = EvalTeamGuarantee(graph, budget, 1, false, resourcecoverage, resourceprob, resourcecost,true);	
			try {
				PrintWriter out_evguar_div = new PrintWriter(new BufferedWriter(new FileWriter(
						"results_with_prob_random_40_evalguar_div_budget_"+budget+".csv", true)));
				out_evguar_div.println(results_evguar_div);
				out_evguar_div.close();
			} catch (IOException e) {
			}

			teamNode.setTeams(resourcecost,resourcecoverage,resourceprob,true);
			String results_opt = PQSearch(graph, budget, 1, false);	
			try {
				PrintWriter out_opt = new PrintWriter(new BufferedWriter(new FileWriter(
						"results_with_prob_random_40_opt_budget_"+budget+".csv", true)));
				out_opt.println(results_opt);
				out_opt.close();
			} catch (IOException e) {
			}
			}
