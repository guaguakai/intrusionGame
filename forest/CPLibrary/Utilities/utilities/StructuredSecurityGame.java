...
public class StructuredSecurityGame {
  ...
  public StructuredSecurityGame(SecurityGame sg){
	  this.nTargets = sg.numTargets;
	  this.nDefenseTypes = 1;
	  this.nDefenders = new int[1];
	  this.nDefenders[0] = sg.numResources;
	  this.payoffs = new SSGPayoffs[nTargets];
	  
	  List<SingleObjectiveTarget> sgList = new ArrayList<SingleObjectiveTarget>(sg.setTargets);
	  Collections.sort(sgList);

	  for (int target = 0; target < nTargets; target++) {
	      SSGPayoffs tmp = new SSGPayoffs(false);
	      tmp.setAttackerCoveredPayoff(sgList.get(target).attCovPayoff);
	      tmp.setAttackerUncoveredPayoff(sgList.get(target).attUncovPayoff);
	      tmp.setDefenderCoveredPayoff(sgList.get(target).defCovPayoff);
	      tmp.setDefenderUncoveredPayoff(sgList.get(target).defUncovPayoff);
	      setPayoffs(target, tmp);
	   }
  }
