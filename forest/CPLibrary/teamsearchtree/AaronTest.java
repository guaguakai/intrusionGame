...
public class AaronTest {
	...
	public static void runRugged() throws LPSolverException,
		...
		PrintWriter output = new PrintWriter(new File("outputtest.txt"));
		...
		while(scanner.hasNext()){
			...
			//ruggedcutoff
			/*RuggedCutoff ruggedObj = new RuggedCutoff(uspObj, 5);
			ruggedObj.run();*/
	
			uspObj.getGraph().edgeSet().size();		
			timeforteam = System.currentTimeMillis()-starttimeteam;
	
			output.print("Team: "+teamName+" Payoff: "+ruggedObj.getGameValue()+" Number of Iterations: "+ruggedObj.getNumberOfIterations()+" Team RunTime: "+(long)(timeforteam/(long)1000));
			output.println();
			System.out.println("Team: "+teamName+" Payoff: "+ruggedObj.getGameValue());
			teamName = "";
		}
