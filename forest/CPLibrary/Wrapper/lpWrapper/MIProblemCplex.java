...
//			cplex.setParam(IloCplex.IntParam.RootAlg, IloCplex.Algorithm.Primal);
	...
	public int addAndSetRowAt(String name,int index, BOUNDS_TYPE boundType,
			double lowerBound, double upperBound) {
		try {
			IloRange newRow;
			...
			rows.put(index, newRow);
			rowsInModel.put(index, ROW_STATUS.ENABLED);
			this.numRows++;
		} catch (IloException e) {
			e.printStackTrace();
