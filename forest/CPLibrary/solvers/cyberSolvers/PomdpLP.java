...
public class PomdpLP extends CPProblem{
	...
	public PomdpLP(AbstractBaseGraph graph){
		super();
		network=graph;
	...
	public void addDecisonVariables(){
		...
		for(int t=0;t<timeHorizon;t++){
			for(INode n : nodeList){		
				...
				this.addNumDecisionVar("p"+n.getId()+","+t, 0, 1);
			...
			for(INode n : nodeList){	
				...
				for(INode n2 : nodeList){
					...
					if(n.equals(n2)){
						...
						IloNumVar p = this.numMap.get("p"+n2.getId()+","+(t-1));
