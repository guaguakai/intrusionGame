...
public class Backup2CompactDefender extends MIProblem {
	 ...
	 */
	public Backup2CompactDefender(TeamBuildingProblem usProblem, HashMap<String,Object[][]> constraints) {
		super();
	...
	public void setObjective() {
		...
		final int index = this.usProblem.getTotalResources()*numTargets;
	...
	public void setColBounds() {
		...
		for (int i=0;i<this.usProblem.getNumDefenderResources().get(k);i++){	
			for ( INode v: targetList) {			
					...
					addAndSetColumn("Path"+(v.getId())+","+(k+1)+","+(i+1),BOUNDS_TYPE.DOUBLE, 0, Double.POSITIVE_INFINITY, VARIABLE_TYPE.INTEGER, 0);
