...
public class CopyofMatrixCompactDefender extends MIProblem {
	...
	public void setObjective() {
		...
		final int index = (this.usProblem.getNumResourceTypes()+1)*((numIntersection)*(numIntersection+1))/2	;
	...
	public CompactAllocation setTargetCoverage(){
		...
		for (int j=i;j<numIntersection;j++) {	
			...
			for (int k=0;k<this.usProblem.getNumResourceTypes();k++){
				double d = getColumnPrimal(this.varMap.get("Ip("+(i)+","+(j)+")"+(k+1)));
				if(getColumnPrimal(this.varMap.get("Ip("+(i)+","+(j)+")"+(k+1))) > Configuration.EPSILON){
	...
	public void addConstraints(NodePair toConstrain, int k){	
		
		int coverage = overflowMap.get(toConstrain).get(k).size();
		
		List<Integer> RowIndices = new ArrayList<Integer>();
		List<Double> RowValues = new ArrayList<Double>();
	
		for(NodePair np : overflowMap.get(toConstrain).get(k)){
			int i = adversaryPaths.indexOf(np.pathset[0]);
			int j = adversaryPaths.indexOf(np.pathset[1]);
			
			RowIndices.add(this.varMap.get("Ip("+(i)+","+(j)+")"+(k+1)));
			RowValues.add(1.0);
		}
		
		int i = adversaryPaths.indexOf(toConstrain.pathset[0]);
		int j = adversaryPaths.indexOf(toConstrain.pathset[1]);
		
		RowIndices.add(this.varMap.get("Ip("+(i)+","+(j)+")"+(k+1)));
		RowValues.add(1.0);
		
		addAndSetRow("Constrain("+(i)+","+(j)+")", BOUNDS_TYPE.UPPER, coverage, coverage); //or fixed
		rowMap.put("Constrain("+(i)+","+(j)+")", this.numRows);		
		
		setMatRow(this.rowMap.get("Constrain("+(i)+","+(j)+")"), RowIndices, RowValues);
		RowIndices.clear();
		RowValues.clear();
	}
	@Override
	public void generateData() {
		...
		for (int k=0;k<this.usProblem.getNumResourceTypes();k++){
			...
			for (int j=i;j<numIntersection;j++) {	
				...
				int index = this.varMap.get("Ip("+(i)+","+(j)+")"+(k+1));
				RowIndices.add(index);
				RowValues.add(1.0);
			}
			}
			int index = this.varMap.get("Resource"+","+(k+1));
			RowIndices.add(index);
			RowValues.add(-1.0);
		...
		for (int j=i;j<numIntersection;j++) {
			...
			RowIndices.add(this.varMap.get("IP("+(i)+","+(j)+")"));
			RowValues.add(1.0);
			...
			for (int k=0;k<this.usProblem.getNumResourceTypes();k++){ 	
				...
				if(coverage>=0 && i!=j && Math.abs(n.distance)<=coverage){ //if it is possible to cover the intersection with resource k
					...
					for (int j2=i2;j2<numIntersection;j2++) {
						...
						if(i==i2&&j==j2){
							index = this.varMap.get("Ip("+(i2)+","+(j2)+")"+(k+1));
						...
						//intersection ij can be covered by overflow from other adjacent covered intersections	
							...
							if(shared!=null && (n2.distance + n.distance-1)<=coverage){
								INode other1 = n.otherNode(shared);
								INode other2 = n2.otherNode(shared);
								...
								for (int j3=i3;j3<numIntersection;j3++) {
										n3 = distances[i3][j3];
										if( (n3.nodeset[0] == other1 || n3.nodeset[0] == other2) && (n3.nodeset[1] == other1 || n3.nodeset[1] == other2) ){
											break;
										}
								}}
			...
			setMatRow(this.rowMap.get("IntersectionValue("+(i)+","+(j)+")"), RowIndices, RowValues);
	...
	public void setColBounds() {
		...
		for (int j=i;j<numIntersection;j++) {	
			for (int k=0;k<this.usProblem.getNumResourceTypes();k++){ //for each resource type
				...
				addAndSetColumn("Ip("+(i)+","+(j)+")"+(k+1),BOUNDS_TYPE.DOUBLE, 0, bound, VARIABLE_TYPE.INTEGER, 0);
				this.varMap.put("Ip("+(i)+","+(j)+")"+(k+1), this.numCols);
			}
