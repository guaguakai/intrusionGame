...
public class RuggedBetterResponses extends Rugged {
	...
	public RuggedBetterResponses(TeamBuildingProblem usProblem) {
		super(usProblem);
		...
		this.isProbability = usProblem.isProbability;
	}
		...
		while (lstDA.size() < count) {
			...
			for(int i=0; i < this.usProblem.getNumResourceTypes();i++){
				for(int j =0;j<this.usProblem.getNumDefenderResources().get(i);j++){
					for(int k=0;k<this.usProblem.ResourceCoverage.get(i);k++){
						//lstEdges.get(0);
						//this.usProblem.nextE(lstEdges.get(0));	//need to finish
					}
				}
			}
	...
	*/
	...
	public Pair<DefenderAllocation, Double> getBetterDefenderResponseNP() {
		...
		for(int i=0;i<this.usProblem.getNumResourceTypes();i++){
			...
			for(int j=0;j<this.usProblem.getNumDefenderResources().get(i);j++){
				if(apNotCoveredYet.size() > 0){
					...
					ExtDWE temp = newEdge;
					for(int i=1;i<k;i++){
						if(!this.usProblem.getGraph().outgoingEdgesOf(temp.getsource()).isEmpty()){
							this.usProblem.getGraph().outgoingEdgesOf(temp.getsource()).toArray()[0];
					}*/
		...
		if (da.allocationSize() < this.usProblem.getTotalResources()) {
...
*/
	...
	//~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~//
	public Pair<List<AdversaryPath>, Pair<AdversaryPath, Double>> getnewBetterAttackerResponses() {
			...
			for (ExtDWE e : this.usProblem.getGraph().outgoingEdgesOf(u)) {
				...
				for (DefenderAllocation da : remaining.get(u)) {
					if (da.contains(e) && defStrategy.get(da) > 0) {
						uvWeight += defStrategy.get(da)*da.getProbability(e);//addprobhere?
...
*/
	...
	public Set<ExtDWE> getTotalMinCut(){
		Set<ExtDWE> mincut = new HashSet<ExtDWE>();
		for(INode n : usProblem.getTargetNodesSet()){
			MinCut obj = new MinCut(usProblem);
			obj.setTarget(n);
			obj.setSource(usProblem.getSource());
			
			mincut.addAll(obj.getMinCut());
		}
		return mincut;
	}
}
