...
void extract (const IloModel model) const;
 ...
 void end();
  ...
  void dumpModel(const char* filename) const {
    ...
    IlcAssert(filename != 0, "IloCP::dumpModel: empty file name ");
    _dumpModel(filename);
  }
  ...
  void exportModel(const char* filename) const {
    IlcAssert(getImpl() != 0, "IloCP: empty handle");
    IlcAssert(filename != 0, "IloCP::exportModel: empty file name ");
  ...
  void importModel(const char* filename) const {
    IlcAssert(getImpl() != 0, "IloCP: empty handle");
    IlcAssert(filename != 0, "IloCP::AddModel: empty file name");
  ...
  IloInt getValue(const char* intVarName) const {
    IlcAssert(getImpl() != 0, "IloCP::getValue: empty handle.");
    IlcAssert(intVarName, "IloCP::getValue: null pointer argument.");
    return _getValue(intVarName);
  }

  
  IloBool isPresent(const char* intervalVarName) const {
    IlcAssert(getImpl() != 0, "IloCP::isPresent: empty handle.");
    IlcAssert(intervalVarName, "IloCP::isPresent: null pointer argument.");
    return _isPresent(intervalVarName);
  }

  
  IloBool isAbsent(const char* intervalVarName) const {
    IlcAssert(getImpl() != 0, "IloCP::isAbsent: empty handle.");
    IlcAssert(intervalVarName, "IloCP::isAbsent: null pointer argument.");
    return _isAbsent(intervalVarName);
  }

  
  IloInt getStart(const char* intervalVarName) const {
    IlcAssert(getImpl() != 0, "IloCP::getStart: empty handle.");
    IlcAssert(intervalVarName, "IloCP::getStart: null pointer argument.");
    return _getStart(intervalVarName);
  }

  
  IloInt getEnd(const char* intervalVarName) const {
    IlcAssert(getImpl() != 0, "IloCP::getEnd: empty handle.");
    IlcAssert(intervalVarName, "IloCP::getEnd: null pointer argument.");
    return _getEnd(intervalVarName);
  }

  
  IloInt getSize(const char* intervalVarName) const {
    IlcAssert(getImpl() != 0, "IloCP::getSize: empty handle.");
    IlcAssert(intervalVarName, "IloCP::getSize: null pointer argument.");
    return _getSize(intervalVarName);
  }

  
  IloInt getLength(const char* intervalVarName) const {
    IlcAssert(getImpl() != 0, "IloCP::getLength: empty handle.");
    IlcAssert(intervalVarName, "IloCP::getLength: null pointer argument.");
  ...
  void exportConflict(ILOSTD(ostream)& str) const {
    IlcAssert(_impl != 0, "IloCP: empty handle");
    IlcAssert(_hasConflict(), "IloCP::exportConflict: no available conflict");
...
public:
  ...
  IloNum getObjMin() const {
    IlcAssert(_impl != 0, "IloCP: empty handle");
    IlcAssert(_hasObjective(), "IloCP: No objective present");
    return _getObjMin();
  }
 ...
 class Exception : public IloAlgorithm::Exception {
  ...
  IloInt getHeightAtStart(const IloIntervalVar var, const IloCumulFunctionExpr f) const {
    ...
    IlcAssert(isExtracted(f), "IloCP: cumul function expression not extracted");
    IlcAssert(_isFixed(f), "IloCP: cumul function expression not fixed");
    IlcAssert(isExtracted(var), "IloIntervalVar: not extracted.");
    IlcAssert(isFixed(var), "IloIntervalVar: not fixed.");
