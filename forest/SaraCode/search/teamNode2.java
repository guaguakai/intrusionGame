...
public class teamNode2 implements Comparable<teamNode>{
	...
	public teamNode(String name, teamNode parent, int resourceindex, double budget, boolean pad){
		...
		if(pad){
			minCostEdge=Double.MAX_VALUE;
			highProb=0;
			for(int i=0;i<resourcecost.length;i++){
				if(resourcecost[i]/resourcecoverage[i]<minCostEdge){
					minCostEdge=resourcecost[i]/resourcecoverage[i];
				}
				if(resourceprob[i]>highProb){
					highProb=resourceprob[i];
				}
			}
	...
	public void setIncrementalEvaluation(int cutoff) throws MalformedGraphException{
		try{
