...
import teamBuildingSolvers.MinCutOracle;
...
public class Search {
	...
	LinkedList<teamNode> queue = new LinkedList<teamNode>();

	//	public static LinkedList<teamNode> getQueue() {
	//		return queue;
	//	}
	...
	public void getMincutVal(teamNode start){
		try {
			getTeams(start);
		...
		for(teamNode team : queue){
			HashMap<ExtDWE, Integer> minCutEdges = getWeightedMincut(team.teamObj);
			MinCutOracle mc = new MinCutOracle(team.teamObj, minCutEdges);

			mc.loadProblem();
			try {
				mc.solve();
				System.out.println(mc.getCutWeight());
	...
	public void ExpandNode(teamNode n) throws MalformedGraphException, LPSolverException{
		for(int i=0;i<teamNode.resourcecost.length;i++){
			...
			if(checkarray(savedteams, child.getResources()) && child.getValue()<=budget){
				...
				child.setIncrementalEvaluation(3);
	...
	public LinkedList<teamNode> ListTeam(teamNode start) throws MalformedGraphException, LPSolverException{		
		//		double tick = System.currentTimeMillis();
		LinkedList<teamNode> return_queue = new LinkedList<teamNode>();
		queue.add(start);
		while(!queue.isEmpty()){
			teamNode n = queue.pop();			
			if(n.leaf){
